...
int main(int argc, char **argv) {
  ...
  if (argc < 8) {
    ...
    std::clog << "query <space> <index> <queries> <gtruth> <block_distance> "
                 "<k> <Reorder ID>"
              << std::endl;
    ...
    std::clog << "\t <block_distance>: int,int,int,int...,int " << std::endl;
  ...
  int block_distance = atoi(argv[5]);
  ...
  // std::cout<<"Reorder time: "<<duration_r.count()/1000.0<<"
  // seconds."<<std::endl;
  ...
  if (reorder_ID == 1) {
    ...
    std::clog << "Reordering" << std::endl;
    auto start_r = std::chrono::high_resolution_clock::now();
    index.profile_reorder(queries, Nq, 1000,
  ...
  for (int i = 0; i < Nq; i++) {
    ...
    std::vector<std::pair<float, int>> result =
        index.greedy_block_location_search(q, k, block_distance);
