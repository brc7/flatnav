...
int main(int argc, char **argv){
    ...
    if (argc < 4){
        ...
        std::clog<<"\t <index_in>: Filename for input index (uint8 index)."<<std::endl;
        ...
        std::clog<<"\t <index_out>: Filename for output index (uint8 index)."<<std::endl;
    ...
    if (is_profiling){
        ...
        // }
        Index<int, int> index(space, infile);
