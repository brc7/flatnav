...
class Index {
 public:
  // List of currently-supported graph reordering algorithms. See reordering.h for how to implement
  // new reordering methods dist_t is the type of the underlying metric space (i.e. for uint8_t
  // images, dist_t is uint8_t) label_t is a fixed-size both dist_t and label_t must be POD types -
  // we do not support custom classes here
 ...
 private:
  ...
  int M;
  int data_size_bytes;  // size of one data point (we do not support variable-size data e.g.
                        // strings)
  int node_size_bytes;  // Node consists of: ([data] [M links] [data label]). This layout was
                        // selected as
  ...
  // distance_param just contains "dimensionality." While it's often known at compile-time, it can
  // be unpleasant to specify e.g. via preprocessor directives. Also poses issues for Python
  // libraries, which only know dimensionality at runtime
    ...
    while (!candidates.empty()) {
      ...
      for (int i = 0; i < M; i++) {
        if (!is_visited[d_node_links[i]]) {  // if we haven't visited the node yet
          ...
          // Include the node in the buffer if buffer isn't full or if node is closer than a node
  ...
  void selectNeighbors(PriorityQueue& neighbors, const int M) {
    ...
    while (candidates.size() > 0) {
      ...
      for (const dist_node_t& second_pair : saved_candidates) {
        dist_t cur_dist =
            distance(nodeData(second_pair.second), nodeData(current_pair.second), distance_param);
      ...
      if (should_keep_candidate) {
        saved_candidates.push_back(
            current_pair);  // could do neighbors.emplace except we have to iterate through
                            // saved_candidates, and std::priority_queue doesn't support iteration
      }
    }
    // implement my own priority queue, get rid of vector saved_candidates, add directly to
    // neighborqueue earlier
  ...
  void connectNeighbors(PriorityQueue& neighbors, node_id_t new_node_id) {
    ...
    while (neighbors.size() > 0) {
      ...
      if (!is_inserted) {
        ...
        dist_t max_dist =
            distance(nodeData(new_node_id), nodeData(neighbor_node_id), distance_param);
        PriorityQueue candidates;
        candidates.emplace(max_dist, new_node_id);
        for (int j = 0; j < M; j++) {
          if (neighbor_node_links[j] != neighbor_node_id) {
            candidates.emplace(distance(nodeData(neighbor_node_id),
                                        nodeData(neighbor_node_links[j]), distance_param),
  ...
  inline void swap(node_id_t a, node_id_t b, void* temp_data, node_id_t* temp_links,
                   label_t* temp_label) {
    // stash b in temp
  ...
  void relabel(const std::vector<node_id_t>& P) {
    ...
    is_visited.clear();  // a better name in this context would be "is_relocated" (I just didn't
                         // want another VisitedList)
 ...
 public:
  // N is the max number of nodes. M is the max number of edges. Space provides info about data size
  ...
  bool add(void* data, label_t& label, int ef_construction, int n_initializations = 100) {
    // initialization must happen before alloc due to a stupid bug where searchInitialization
    // chooses new_node_id as the initialization since new_node_id has distance 0 (but no links),
    // this bug literally skips the search
  ...
  void save(const std::string& location) {
    ...
    // currently none of this is safe across machines or even across compilers, probably. Really
    // sorry about that.
