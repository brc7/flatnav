...
public:
  // List of currently-supported graph reordering algorithms. See reordering.h
  // for how to implement new reordering methods dist_t is the type of the
  // underlying metric space (i.e. for uint8_t images, dist_t is uint8_t)
  // label_t is a fixed-size both dist_t and label_t must be POD types - we do
  // not support custom classes here
...
private:
  ...
  size_t M;
  size_t data_size_bytes; // size of one data point (we do not support
                          // variable-size data e.g. strings)
  size_t node_size_bytes; // Node consists of: ([data] [M links] [data label]).
                          // This layout was selected as
  ...
  // distance_param just contains "dimensionality." While it's often known at
  // compile-time, it can be unpleasant to specify e.g. via preprocessor
  // directives. Also poses issues for Python libraries, which only know
  // dimensionality at runtime

  VisitedSet is_visited; // remembers which nodes we've visited, to avoid
                         // re-computing distances
  // might be a caching problem - need to profile. I would love to get rid of
  // this in beamSearch.
  ...
  PriorityQueue beamSearch(const void* query, const node_id_t entry_node,
    ...
    while (!candidates.empty()) {
      ...
      for (int i = 0; i < M; i++) {
        if (!is_visited[d_node_links[i]]) { // if we haven't visited the node
                                            // yet
          is_visited.insert(d_node_links[i]);
          dist = distance(query, nodeData(d_node_links[i]), distance_param);
          // Include the node in the buffer if buffer isn't full or if node is
          // closer than a node already in the buffer
  ...
  node_id_t getBlockStart(node_id_t node, node_id_t block_distance) {
    if (node < block_distance) {
      return 0;
    }
    return node - block_distance;
  }

  node_id_t getBlockEnd(node_id_t node, node_id_t block_distance) {
    return std::min<node_id_t>(node + block_distance, cur_num_nodes);
  }

  PriorityQueue greedyBlockSearchSingleIter(const void* query,
                                            node_id_t entry_node,
                                            node_id_t block_distance) {
    PriorityQueue neighbors;

    is_visited.clear();

    // Greedy search until we don't find any closer neighbors.
    node_id_t next_node_to_explore = entry_node;
    dist_t dist_to_query =
        distance(query, nodeData(next_node_to_explore), distance_param);

    while (!is_visited[next_node_to_explore]) {
      is_visited.insert(next_node_to_explore);

      neighbors.emplace(dist_to_query, next_node_to_explore);

      const node_id_t* nbrs = nodeLinks(next_node_to_explore);

      for (size_t i = 0; i < M; i++) {
        if (!is_visited[nbrs[i]]) {
          dist_t dist = distance(query, nodeData(nbrs[i]), distance_param);
          if (dist < dist_to_query) {
            next_node_to_explore = nbrs[i];
            dist_to_query = dist;
          }
        }
      }
    ...
    // Search all nodes around the node found with greedy search.
    node_id_t block_start = getBlockStart(next_node_to_explore, block_distance);
    node_id_t block_end = getBlockEnd(next_node_to_explore, block_distance);

    for (node_id_t block_node = block_start; block_node < block_end;
         block_node++) {
  ...
  PriorityQueue greedyBlockSearch(const void* query, node_id_t entry_node,
                                  node_id_t block_distance) {
    PriorityQueue neighbors;
    ...
    while (1) {
      ...
      node_id_t old_node = next_node_to_explore;
      ...
      if (next_node_to_explore == old_node) {
        // Search all nodes around the node found with greedy search.
        node_id_t block_start =
            getBlockStart(next_node_to_explore, block_distance);
        node_id_t block_end = getBlockEnd(next_node_to_explore, block_distance);
        ...
        for (node_id_t block_node = block_start; block_node < block_end;
          ...
          if (!is_visited[block_node]) {
            ...
            if (dist < dist_to_query) {
              next_node_to_explore = block_node;
              dist_to_query = dist;
            }
          }
        }
  ...
  void reprune(node_id_t node) {
    ...
    for (int i = 0; i < M; i++) {
      if (links[i] != node) {
        dist_t dist =
            distance(nodeData(node), nodeData(links[i]), distance_param);
  ...
  void selectNeighbors(PriorityQueue &neighbors, const int M) {
    ...
    while (candidates.size() > 0) {
      ...
      for (const dist_node_t &second_pair : saved_candidates) {
        dist_t cur_dist =
            distance(nodeData(second_pair.second),
                     nodeData(current_pair.second), distance_param);
      ...
      if (should_keep_candidate) {
        saved_candidates.push_back(
            current_pair); // could do neighbors.emplace except we have to
                           // iterate through saved_candidates, and
                           // std::priority_queue doesn't support iteration
      }
    }
    // implement my own priority queue, get rid of vector saved_candidates, add
    // directly to neighborqueue earlier
    ...
    while (neighbors.size() > 0) {
      ...
      for (int j = 0; j < M; j++) {
        if (neighbor_node_links[j] == neighbor_node_id) {
          // if there is a self-loop, replace the self-loop with the desired
          // link
      ...
      if (!is_inserted) {
        // now, we may to replace one of the links. This will disconnect the old
        // neighbor and create a directed edge, so we have to be very careful.
        // To ensure we respect the pruning heuristic, we construct a candidate
        // set including the old links AND our new one and then prune this
        // candidate set to get the new neighbors
        dist_t max_dist = distance(nodeData(new_node_id),
                                   nodeData(neighbor_node_id), distance_param);
        PriorityQueue candidates;
        candidates.emplace(max_dist, new_node_id);
        for (int j = 0; j < M; j++) {
          if (neighbor_node_links[j] != neighbor_node_id) {
            candidates.emplace(distance(nodeData(neighbor_node_id),
  ...
  inline void swap(node_id_t a, node_id_t b, void* temp_data,
  ...
  void relabel(const std::vector<node_id_t> &P) {
    ...
    is_visited.clear(); // a better name in this context would be "is_relocated"
                        // (I just didn't want another VisitedList)
    ...
    for (node_id_t n = 0; n < cur_num_nodes; n++) {
      if (!is_visited[n]) {
        ...
        while (!is_visited[dest]) {
          ...
          // the value of src remains the same. However, dest needs to change
          // because the node located at src was previously located at dest, and
          // must be relocated to P[dest]
          dest = P[dest];
...
public:
  // N is the max number of nodes. M is the max number of edges. Space provides
  // info about data size and distance function
  Index(SpaceInterface<dist_t>* space, int _N, int _M)
    ...
    node_size_bytes =
        space->get_data_size() + sizeof(node_id_t)*  M + sizeof(label_t);
  ...
  ~Index() { delete[] index_memory; }

  bool add(void* data, label_t &label, int ef_construction,
           int n_initializations = 100) {
    // initialization must happen before alloc due to a stupid bug where
    // searchInitialization chooses new_node_id as the initialization since
    // new_node_id has distance 0 (but no links), this bug literally skips the
    // search
  ...
  std::vector<dist_label_t> search(const void* query, const int K,
                                   int ef_search, int n_initializations = 100) {
  ...
  greedy_block_location_search(const void* query, const int K,
                               node_id_t block_distance,
                               int n_initializations = 100) {
    node_id_t entry_node = searchInitialization(query, n_initializations);
    PriorityQueue neighbors =
        greedyBlockSearch(query, entry_node, block_distance);
    std::vector<dist_label_t> results;
    while (neighbors.size() > K) {
      neighbors.pop();
    }
  ...
  void save(const std::string &location) {
    ...
    // data partition and endian checks. Also save as standard-size integers
    // rather than int or node_id_t (which can be compiler dependent). currently
    // none of this is safe across machines or even across compilers, probably.
  ...
  void flash(std::vector<std::vector<node_id_t>> &outdegree_table) {
    ...
    for (node_id_t node = 0; node < cur_num_nodes; node++) {
      ...
      for (node_id_t &neighbor_node : outdegree_table[node]) {
        dist_t dist =
            distance(nodeData(node), nodeData(neighbor_node), distance_param);
  ...
  void reorder(GraphOrder algorithm) {
    ...
    // List of algorithms (so far): GORDER, IN_DEG, OUT_DEG, RCM, HUB_SORT,
    // HUB_CLUSTER, DBG, BCORDER
    switch (algorithm) {
    case GraphOrder::GORDER:
      P = g_order<node_id_t>(outdegree_table, 40);
      break;
    case GraphOrder::IN_DEG:
      P = indegree_order<node_id_t>(outdegree_table);
      break;
    case GraphOrder::OUT_DEG:
      P = outdegree_order<node_id_t>(outdegree_table);
      break;
    case GraphOrder::RCM:
      P = rcm_order<node_id_t>(outdegree_table);
      break;
    case GraphOrder::RCM_2HOP:
      P = rcm_order_2hop<node_id_t>(outdegree_table);
      break;
    case GraphOrder::HUB_SORT:
      P = hubsort_order<node_id_t>(outdegree_table);
      break;
    case GraphOrder::HUB_CLUSTER:
      P = hubcluster_order<node_id_t>(outdegree_table);
      break;
    case GraphOrder::DBG:
      P = dbg_order<node_id_t>(outdegree_table, 8);
      break;
    case GraphOrder::BCORDER:
      P = bc_order<node_id_t>(outdegree_table, 5);
      break;
    }
    relabel(P);
  }

  void profile_reorder(void* queries, int n_queries, int ef_search,
                       ProfileOrder algorithm) {
    ...
    std::vector<node_id_t> P;
    switch (algorithm) {
    case ProfileOrder::GORDER:
      P = weighted_g_order<node_id_t>(outdegree_table, outdegree_weights, 5);
      break;
    case ProfileOrder::RCM:
      P = weighted_rcm_order<node_id_t>(outdegree_table, outdegree_weights);
      break;
    }
    ...
    while (!candidates.empty()) {
      ...
      for (int i = 0; i < M; i++) {
        if (!is_visited[d_node_links[i]]) { // if we haven't visited the node
          ...
          for (int outdegree_index = 0;
               outdegree_index < outdegree_table[d_node.second].size();
               outdegree_index++) {
            if (outdegree_table[d_node.second][outdegree_index] ==
                d_node_links[i]) {
  ...
  std::vector<int> location_search(const void* query, const int K,
                                   int ef_search, int n_initializations = 100) {
    node_id_t entry_node = searchInitialization(query, n_initializations);
    PriorityQueue neighbors = beamSearch(query, entry_node, ef_search);
    ...
    std::sort(results.begin(), results.end(),
              [](const dist_node_t &left, const dist_node_t &right) {
                return left.first < right.first;
              });
